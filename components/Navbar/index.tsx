...
const ContactButton = ({ className }: { className: string }) => {
  return <Link href={"/#contact"} className={twMerge("px-4 md:px-5 py-2 w-fit border-2 border-primary bg-primary hover:bg-primary-hover transition-all rounded-sm text-dark-gray", className)}>Contact</Link>
};
...
export default function Navbar() {
  ...
  return (<header className={"fixed w-full right-0 bg-dark-gray bg-opacity-80 backdrop-blur-md z-10 transition-all ease-out duration-500 " + `${(isNavbarVisible || isOpen) ? "top-0" : "-top-20"}`}>
